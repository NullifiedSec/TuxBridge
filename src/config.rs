...
pub struct AuthConfig {
    /// Backwards-compatible administrator key. Existing installs keep working.
    #[serde(default = "default_api_key_env")]
    pub api_key_env: String,
    /// Optional named, role-scoped principals for separate GPT occupations.
    #[serde(default)]
    pub principals: BTreeMap<String, PrincipalConfig>,
}
impl Default for AuthConfig {
    fn default() -> Self {
        Self { api_key_env: default_api_key_env(), principals: BTreeMap::new() }
    }
}
...
impl Default for SecurityConfig {
    fn default() -> Self {
        Self { profile: SecurityProfile::Default, default_command_allowlist: default_command_allowlist() }
    }
}
...
pub struct LspConfig {
    #[serde(default)] pub servers: BTreeMap<String, LspServerConfig>,
}
...
impl Config {
    ...
    pub fn auth_principals(&self) -> Result<Vec<LoadedPrincipal>, ConfigError> {
        let mut loaded = Vec::with_capacity(self.auth.principals.len() + 1);
        loaded.push(LoadedPrincipal {
            name: "admin".into(),
            key: read_secret(&self.auth.api_key_env)?,
            roles: vec![AuthRole::Admin],
        });
        for (name, principal) in &self.auth.principals {
            loaded.push(LoadedPrincipal {
                name: name.clone(),
                key: read_secret(&principal.api_key_env)?,
                roles: principal.roles.clone(),
            });
        }
        for left in 0..loaded.len() {
            for right in left + 1..loaded.len() {
                if loaded[left].key == loaded[right].key {
                    return Err(ConfigError::Invalid(format!(
                        "auth principals {:?} and {:?} resolve to the same API key",
                        loaded[left].name, loaded[right].name
                    )));
                }
            }
        }
    ...
    fn validate(&self) -> Result<(), ConfigError> {
        ...
        validate_env_name("auth.api_key_env", &self.auth.api_key_env)?;
        for (name, principal) in &self.auth.principals {
            if name.trim().is_empty() || name.chars().any(char::is_whitespace) {
                return Err(ConfigError::Invalid("auth principal names must be non-empty and whitespace-free".into()));
            }
            validate_env_name(&format!("auth.principals.{name}.api_key_env"), &principal.api_key_env)?;
            if principal.roles.is_empty() {
                return Err(ConfigError::Invalid(format!("auth principal {name:?} must have at least one role")));
            }
            let unique = principal.roles.iter().copied().collect::<BTreeSet<_>>();
            if unique.len() != principal.roles.len() {
                return Err(ConfigError::Invalid(format!("auth principal {name:?} contains duplicate roles")));
            }
        }
        ...
        for path in &self.approvals.required_paths {
            if !path.starts_with("/v1/") || path.chars().any(char::is_whitespace) {
                return Err(ConfigError::Invalid(format!("approval path {path:?} must be an exact whitespace-free /v1/... path")));
            }
            if path.starts_with("/v1/approvals/") {
                return Err(ConfigError::Invalid("approval management endpoints cannot require their own approval".into()));
            }
        }
...
fn read_secret(name: &str) -> Result<String, ConfigError> {
    let value = env::var(name).map_err(|_| ConfigError::MissingSecret(name.to_owned()))?;
    if value.len() < 32 || value.trim() != value || value.chars().any(char::is_control) {
        return Err(ConfigError::Invalid(format!(
            "API key from {name} must be at least 32 characters and contain no surrounding whitespace or control characters"
        )));
    }
    Ok(value)
}

fn validate_env_name(label: &str, value: &str) -> Result<(), ConfigError> {
    if value.trim().is_empty() || value.chars().any(char::is_whitespace) {
        Err(ConfigError::Invalid(format!("{label} must be a non-empty environment variable name")))
    } else {
        Ok(())
    }
}
...
mod tests {
    ...
    fn applies_defaults() {
        ...
        assert!(config.approvals.required_paths.is_empty());
    }
}
