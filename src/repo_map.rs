...
fn scan(root:&Path,path:&Path,acc:&mut Accumulator)->Result<(),ApiError>{
    ...
    if let Some(first)=Path::new(&rel).components().next().map(|c|c.as_os_str().to_string_lossy().into_owned()){if matches!(first.as_str(),"src"|"app"|"lib"|"packages"|"crates"|"apps"|"internal"|"cmd"){acc.source_roots.insert(first);}if matches!(first.as_str(),"test"|"tests"|"spec"|"specs"|"__tests__"){acc.test_roots.insert(first);}}
