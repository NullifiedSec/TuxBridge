...
use crate::{config::AuthRole, state::AppState};
...
pub async fn require_api_key(
    ...
    let Some(credential) = state
        .principals
        .iter()
        .find(|principal| constant_time_eq(provided.as_bytes(), principal.key.as_bytes()))
    else {
        return unauthorized();
    };
...
fn developer_allows(method: &Method, path: &str) -> bool {
    if method == Method::GET {
        return matches!(
            path,
            "/v1/lsp/servers" | "/v1/code/sessions"
        ) || session_path(path, "");
    }
    ...
    matches!(
        ...
        "/v1/project/inspect"
            ...
            | "/v1/git/commit"
            | "/v1/git/push"
    ) || session_path(path, "finalize")
        || session_path(path, "rollback")
}

fn reviewer_allows(method: &Method, path: &str) -> bool {
    if method == Method::GET {
        return matches!(path, "/v1/lsp/servers" | "/v1/workspaces")
            || (path.starts_with("/v1/workspaces/") && !path["/v1/workspaces/".len()..].contains('/'));
    }
    method == Method::POST
        && matches!(
            path,
...
fn operator_allows(method: &Method, path: &str) -> bool {
    ...
    matches!(
        ...
        "/v1/fs/list"
            ...
            | "/v1/git/fetch"
            | "/v1/git/pull"
    ) || approval_path(path, "approve")
        || approval_path(path, "deny")
}
...
mod tests {
    ...
    fn reviewer_cannot_mutate() {
        assert!(reviewer_allows(&Method::POST, "/v1/git/diff"));
        assert!(!reviewer_allows(&Method::POST, "/v1/git/commit"));
        assert!(!reviewer_allows(&Method::POST, "/v1/code/edit-plan"));
    }

    #[test]
    fn developer_can_code_but_not_operate_host() {
        assert!(developer_allows(&Method::POST, "/v1/code/edit-plan"));
        assert!(developer_allows(&Method::POST, "/v1/git/commit"));
        assert!(!developer_allows(&Method::GET, "/v1/system/processes"));
    }

    #[test]
    fn operator_can_manage_jobs_and_approvals() {
        assert!(operator_allows(&Method::DELETE, "/v1/jobs/job-1"));
        assert!(operator_allows(&Method::POST, "/v1/approvals/a-1/approve"));
        assert!(!operator_allows(&Method::POST, "/v1/lsp/rename"));
    }
}
