...
pub async fn code_context(
    ...
    let workspace = workspace(&state, &request.workspace, false)?;
    let (root, target) = existing_file(workspace, &request.path)?;
    ...
    let start_line = requested_start
        .saturating_sub(request.context_before.unwrap_or(20))
        .max(1);
...
pub async fn code_references(
    ...
    let start = match request.path.as_deref() {
        Some(path) => existing_path(&root, path)?,
        None => root.clone(),
    };
    let max_results = request.max_results.unwrap_or(500).clamp(1, MAX_REFERENCES);
    let mut result = CodeReferencesResponse {
        identifier: request.identifier.clone(),
        references: Vec::new(),
        files_scanned: 0,
        truncated: false,
    };
    scan_references(&root, &start, &request.identifier, max_results, &mut result)?;
    Ok(Json(result))
}
...
pub async fn code_edit_plan(
    ...
    let workspace = workspace(&state, &request.workspace, true)?;
    let root = canonical_root(workspace)?;
    let mut seen = BTreeSet::new();
    let mut prepared = Vec::with_capacity(request.files.len());

    // Preflight all files and hashes before touching disk.
    for file in &request.files {
        ...
        let target = existing_from_root(&root, &file.path)?;
        let path = relative(&root, &target);
        if !seen.insert(path.clone()) {
            return Err(ApiError::BadRequest(format!("duplicate edit target {path:?}")));
        }
        reject_symlink(&target)?;
        let original_bytes = read_bounded(&target)?;
        let old_sha256 = digest(&original_bytes);
        if !file.expected_sha256.eq_ignore_ascii_case(&old_sha256) {
            return Err(ApiError::Conflict(format!(
                "file changed: {path} expected sha256 {}, current sha256 {old_sha256}",
                file.expected_sha256
            )));
        }
        let original = String::from_utf8(original_bytes)
            .map_err(|_| ApiError::Unsupported(format!("{path} is not valid UTF-8 text")))?;
        let updated = apply_edits(original.clone(), &file.edits)?;
        if updated.len() > MAX_FILE_BYTES {
            return Err(ApiError::BadRequest(format!("updated file {path} exceeds {MAX_FILE_BYTES} bytes")));
        }
        prepared.push(PreparedEdit {
            target,
            path,
            permissions: fs::metadata(&root.join(&file.path)).map_err(map_io)?.permissions(),
            old_sha256,
            original,
            updated,
        });
    ...
    if !request.dry_run {
        // Each file replace is atomic. The plan is fully preflighted, but multiple
        // files are not claimed to be one filesystem-level atomic transaction.
        for item in &prepared {
            atomic_write(&item.target, item.updated.as_bytes(), item.permissions.clone())?;
        }
    }
...
pub async fn discover_code_tasks(
    ...
    if root.join("package.json").is_file() {
        discover_package_scripts(&root, &mut tasks);
    }
...
fn result_for(item: &PreparedEdit) -> FileEditResult {
    ...
    FileEditResult {
        ...
        old_sha256: item.old_sha256.clone(),
        new_sha256: digest(item.updated.as_bytes()),
        changed: item.original != item.updated,
        additions: new_end.saturating_sub(prefix),
        deletions: old_end.saturating_sub(prefix),
        preview: preview(&old_lines, &new_lines, prefix, old_end, new_end),
    }
}

fn preview(old: &[&str], new: &[&str], start: usize, old_end: usize, new_end: usize) -> String {
    let mut out = format!("@@ line {} @@\n", start + 1);
    for line in &old[start.saturating_sub(3)..start] {
    ...
    for line in new.iter().skip(new_end).take(3) {
        out.push_str("  "); out.push_str(line); out.push('\n');
    }
    if out.len() > 32 * 1024 {
    ...
    for edit in edits {
        text = match edit {
            CodeEdit::ReplaceExact { old, new } => {
                if old.is_empty() { return Err(ApiError::BadRequest("replace_exact old text must not be empty".into())); }
                let count = text.match_indices(old).count();
                if count != 1 { return Err(ApiError::Conflict(format!("replace_exact requires exactly one match, found {count}"))); }
                text.replacen(old, new, 1)
            }
            CodeEdit::ReplaceLines { start_line, end_line, new } => replace_lines(&text, *start_line, *end_line, new)?,
            CodeEdit::InsertBefore { line, new } => insert_line(&text, *line, new, false)?,
            CodeEdit::InsertAfter { line, new } => insert_line(&text, *line, new, true)?,
        };
    }
    Ok(text)
}

fn replace_lines(text: &str, start: usize, end: usize, new: &str) -> Result<String, ApiError> {
    if start == 0 || end < start { return Err(ApiError::BadRequest("invalid line range".into())); }
    let lines = split_lines(text);
    if end > lines.len() { return Err(ApiError::Conflict(format!("line range {start}..={end} exceeds file length {}", lines.len()))); }
    let mut out = lines[..start - 1].concat();
    out.push_str(new);
...
fn insert_line(text: &str, line: usize, new: &str, after: bool) -> Result<String, ApiError> {
    if line == 0 { return Err(ApiError::BadRequest("line numbers are 1-based".into())); }
    let lines = split_lines(text);
    if line > lines.len().max(1) { return Err(ApiError::Conflict(format!("line {line} exceeds file length {}", lines.len()))); }
    let index = if after { line.min(lines.len()) } else { line.saturating_sub(1).min(lines.len()) };
    let mut out = lines[..index].concat();
    out.push_str(new);
...
fn scan_references(root: &Path, path: &Path, identifier: &str, max: usize, out: &mut CodeReferencesResponse) -> Result<(), ApiError> {
    if out.references.len() >= max { out.truncated = true; return Ok(()); }
    let metadata = fs::symlink_metadata(path).map_err(map_io)?;
    if metadata.file_type().is_symlink() { return Ok(()); }
    if metadata.is_file() {
        if metadata.len() > MAX_SCAN_FILE_BYTES || binary_extension(path) { return Ok(()); }
        out.files_scanned += 1;
        let Ok(text) = fs::read_to_string(path) else { return Ok(()); };
        for (line_index, line) in text.lines().enumerate() {
            for column in identifier_columns(line, identifier) {
                out.references.push(CodeReference {
        ...
        for entry in fs::read_dir(path).map_err(map_io)? {
            ...
            if matches!(name.as_ref(), ".git" | "node_modules" | "target" | ".nuxt" | ".next" | "dist" | "build" | "vendor") { continue; }
            scan_references(root, &entry.path(), identifier, max, out)?;
...
fn identifier_columns(line: &str, identifier: &str) -> Vec<usize> {
    ...
    while offset <= line.len() {
        let Some(found) = line[offset..].find(identifier) else { break; };
        let start = offset + found;
        let end = start + identifier.len();
        let left_ok = line[..start].chars().next_back().is_none_or(|ch| !ident_char(ch));
        let right_ok = line[end..].chars().next().is_none_or(|ch| !ident_char(ch));
        if left_ok && right_ok { columns.push(start); }
        offset = end;
...
fn validate_identifier(identifier: &str) -> Result<(), ApiError> {
    if identifier.is_empty() || identifier.len() > 256 || !identifier.chars().all(ident_char) || identifier.chars().next().is_some_and(|ch| ch.is_ascii_digit()) {
        Err(ApiError::BadRequest("identifier must be a simple programming-language identifier".into()))
    } else { Ok(()) }
}
...
fn detect_symbol(language: &str, line: &str) -> Option<(String, String)> {
    ...
    let patterns: &[(&str, &str)] = match language {
        ...
        "javascript" | "typescript" => &[("export async function ", "function"), ("export function ", "function"), ("async function ", "function"), ("function ", "function"), ("export class ", "class"), ("class ", "class"), ("export interface ", "interface"), ("interface ", "interface"), ("export type ", "type"), ("type ", "type")],
    ...
    for (prefix, kind) in patterns {
        if let Some(rest) = line.strip_prefix(prefix) {
            let name = rest.split(|ch: char| !ident_char(ch)).next().unwrap_or("");
            if !name.is_empty() { return Some(((*kind).into(), name.into())); }
        }
    }
    None
}
...
fn language_for(path: &Path) -> &'static str {
    match path.extension().and_then(|x| x.to_str()).unwrap_or("") {
        ...
        "yaml" | "yml" => "yaml", _ => "text",
    }
}

fn binary_extension(path: &Path) -> bool {
    matches!(path.extension().and_then(|x| x.to_str()).unwrap_or("").to_ascii_lowercase().as_str(),
        "png" | "jpg" | "jpeg" | "gif" | "webp" | "ico" | "pdf" | "zip" | "gz" | "xz" | "bz2" | "7z" | "tar" | "wasm" | "exe" | "dll" | "so" | "a" | "o" | "class" | "jar" | "lockb")
}

fn discover_package_scripts(root: &Path, tasks: &mut Vec<CodeTask>) {
    let manager = if root.join("bun.lock").is_file() || root.join("bun.lockb").is_file() { "bun" }
        else if root.join("pnpm-lock.yaml").is_file() { "pnpm" }
        else if root.join("yarn.lock").is_file() { "yarn" } else { "npm" };
    let Ok(raw) = fs::read_to_string(root.join("package.json")) else { return; };
    let Ok(value) = serde_json::from_str::<serde_json::Value>(&raw) else { return; };
    let Some(scripts) = value.get("scripts").and_then(|x| x.as_object()) else { return; };
    for name in ["typecheck", "check", "lint", "test", "build", "format"] {
        ...
        tasks.push(CodeTask {
            id: format!("js-{name}"), label: format!("package script: {name}"), argv,
...
fn task(id: &str, label: &str, argv: &[&str], source: &str) -> CodeTask {
    CodeTask {
        id: id.into(), label: label.into(), argv: argv.iter().map(|x| (*x).into()).collect(), source: source.into(),
...
fn workspace<'a>(state: &'a AppState, name: &str, write: bool) -> Result<&'a WorkspaceConfig, ApiError> {
    let workspace = state.config.workspaces.get(name)
        .ok_or_else(|| ApiError::NotFound(format!("workspace {name:?} is not configured")))?;
    let allowed = if write { workspace.capabilities.fs_write } else { workspace.capabilities.fs_read };
    if allowed { Ok(workspace) } else { Err(ApiError::Forbidden(format!("workspace {name:?} does not allow filesystem {}", if write { "writes" } else { "reads" }))) }
}
...
fn canonical_root(workspace: &WorkspaceConfig) -> Result<PathBuf, ApiError> {
    ...
    if root.is_dir() { Ok(root) } else { Err(ApiError::BadRequest("workspace root is not a directory".into())) }
}

fn existing_file(workspace: &WorkspaceConfig, requested: &str) -> Result<(PathBuf, PathBuf), ApiError> {
    let root = canonical_root(workspace)?;
    let target = existing_from_root(&root, requested)?;
    if fs::metadata(&target).map_err(map_io)?.is_file() { Ok((root, target)) } else { Err(ApiError::BadRequest("path is not a regular file".into())) }
}

fn existing_from_root(root: &Path, requested: &str) -> Result<PathBuf, ApiError> {
    let path = validate_relative(requested, false)?;
...
fn existing_path(root: &Path, requested: &str) -> Result<PathBuf, ApiError> {
    let path = validate_relative(requested, true)?;
    let target = fs::canonicalize(root.join(path)).map_err(map_io)?;
    ensure_within(root, &target)?;
    Ok(target)
}

fn validate_relative(requested: &str, allow_empty: bool) -> Result<&Path, ApiError> {
    if requested.is_empty() { return if allow_empty { Ok(Path::new(".")) } else { Err(ApiError::BadRequest("path must not be empty".into())) }; }
    let path = Path::new(requested);
    if path.is_absolute() { return Err(ApiError::BadRequest("path must be relative to the workspace".into())); }
...
fn ensure_within(root: &Path, target: &Path) -> Result<(), ApiError> {
    if target.starts_with(root) { Ok(()) } else { Err(ApiError::Forbidden("resolved path escapes workspace root".into())) }
}

fn reject_symlink(path: &Path) -> Result<(), ApiError> {
    let metadata = fs::symlink_metadata(path).map_err(map_io)?;
    if metadata.file_type().is_symlink() { Err(ApiError::Forbidden("code tools do not mutate through symlinks".into())) } else { Ok(()) }
}

fn read_bounded(path: &Path) -> Result<Vec<u8>, ApiError> {
    let metadata = fs::metadata(path).map_err(map_io)?;
    if metadata.len() > MAX_FILE_BYTES as u64 { Err(ApiError::BadRequest(format!("file exceeds {MAX_FILE_BYTES} bytes"))) } else { fs::read(path).map_err(map_io) }
}

fn atomic_write(target: &Path, content: &[u8], permissions: fs::Permissions) -> Result<(), ApiError> {
    let parent = target.parent().ok_or_else(|| ApiError::BadRequest("target must have a parent directory".into()))?;
...
mod tests {
    ...
    #[test]
    fn references_respect_identifier_boundaries() {
        assert_eq!(identifier_columns("foo foobar foo", "foo"), vec![0, 11]);
    }

    #[test]
    fn replace_lines_is_one_based() {
        assert_eq!(replace_lines("a\nb\nc\n", 2, 2, "B\n").unwrap(), "a\nB\nc\n");
    }

    #[test]
    fn exact_replace_refuses_ambiguity() {
        let edits = vec![CodeEdit::ReplaceExact { old: "x".into(), new: "y".into() }];
