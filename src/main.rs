...
use axum::{Json, Router, extract::DefaultBodyLimit, middleware, routing::{get, post}};
use serde::Serialize;
...
async fn health()->Json<HealthResponse>{Json(HealthResponse{status:"ok",service:"tuxbridge",version:env!("CARGO_PKG_VERSION")})}

#[tokio::main]
async fn main()->Result<(),Box<dyn std::error::Error>>{
    let config_path=env::var_os("TUXBRIDGE_CONFIG").map(PathBuf::from).unwrap_or_else(||PathBuf::from("tuxbridge.toml"));
    let config=Config::load(&config_path)?;let listen:SocketAddr=config.server.listen.parse()?;let max_body_bytes=config.limits.max_body_bytes;let state=AppState::new(config)?;
    ...
    let protected=Router::new()
        ...
        .route("/v1/code/map",post(repo_map::repository_map))
        .route("/v1/code/structure",post(structural::structure))
        .route("/v1/code/node-at",post(structural::node_at))
        .route("/v1/code/replace-node",post(structural::replace_node))
        .route("/v1/code/verification-plan",post(verification::verification_plan))
        .route("/v1/code/sessions",get(sessions::list_sessions).post(sessions::create_session))
        .route("/v1/code/sessions/{id}",get(sessions::get_session))
        .route("/v1/code/sessions/{id}/checkpoint",post(sessions::checkpoint))
        .route("/v1/code/sessions/{id}/refresh",post(sessions::refresh))
        .route("/v1/code/sessions/{id}/finalize",post(sessions::finalize))
        .route("/v1/code/sessions/{id}/rollback",post(sessions::rollback))
        ...
        .route("/v1/jobs",get(command::list_jobs))
        .route("/v1/jobs/{id}",get(command::get_job).delete(command::cancel_job))
    ...
    let app=Router::new().route("/health",get(health)).route("/ui",get(dashboard::dashboard)).merge(protected).layer(DefaultBodyLimit::max(max_body_bytes)).layer(middleware::from_fn_with_state(state.clone(),hardening::protect_host)).with_state(state);
    let listener=TcpListener::bind(listen).await?;println!("tuxbridge listening on {listen}");axum::serve(listener,app).with_graceful_shutdown(shutdown_signal()).await?;Ok(())
}
