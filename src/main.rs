...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let protected = Router::new()
        ...
        .route(
            ...
            post(verification::verification_plan),
        )
        .route(
            "/v1/code/sessions",
        ...
        .route(
            "/v1/code/sessions/{id}/checkpoint",
            post(sessions::checkpoint),
        )
        .route(
            "/v1/code/sessions/{id}/refresh",
        ...
        .route(
            "/v1/agent/workspaces/find",
            post(sessions::find_workspaces),
        )
        .route(
            "/v1/agent/sessions/select",
            post(sessions::select_workspace),
        )
        .route(
            "/v1/agent/sessions/choose",
            post(sessions::choose_session),
        )
        .route(
            "/v1/agent/sessions/summary",
            post(sessions::set_summary),
        )
        .route("/v1/agent/sessions/plan", post(sessions::set_plan))
        .route("/v1/agent/sessions/todo", post(sessions::todo))
        .route(
    ...
    let app = Router::new()
        ...
        .route("/docs", get(docs::scalar_reference))
        .route("/openapi-agent-v2.yaml", get(docs::agent_openapi))
        .merge(protected)
